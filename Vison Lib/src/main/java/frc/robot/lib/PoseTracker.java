...
public class PoseTracker {
    ...
    public class Moudule {
        ...
        private boolean accleTrust = true;
        private boolean skidTrust = true;


        public void setAccleTrust(boolean accleTrust) {
            this.accleTrust = accleTrust;
        }
        public void setSkidTrust(boolean skidTrust) {
            this.skidTrust = skidTrust;
        }
        public boolean isTrust() {
            return accleTrust && skidTrust;
        }
    ...
    public class RobotTracker {
        ...
        private static List<Moudule> modules;
        private static GyroValues gyroValues;
        public static Pose2d pose;
        private static boolean UseAcclDataForTrustingPose;
        private static double AcclThreashould = 1;

        public static void setAcclThreashould(double acclThreashould) {
            AcclThreashould = acclThreashould;
        }

        public static void setUseAcclDataForTrustingPose(boolean useAcclDataForTrustingPose) {
            UseAcclDataForTrustingPose = useAcclDataForTrustingPose;
        }
        ...
        public static double getSkidRatio(){

            SwerveDriveKinematics getter = new SwerveDriveKinematics();
            SwerveModuleState[] swerveStatesRotationalPart = getter.toSwerveModuleStates(new ChassisSpeeds(0, 0, gyroValues.getYawVel()));
        ...
        public static Translation2d getOdmetryOffsetToField(){
            ...
            for(Moudule module : modules){
                if(module.isTrust()){
                    sampleSize++;
                    offset.plus(module.getTransformOffset().plus(offset));
                }

                module.setSkidTrust(true);
            }
        ...
        private static void UseAcclDataToTrustModules(double Threashould){
            ...
            for(Moudule module : modules){
                ...
                Translation2d moudleAccl = module.getAccleration();
                
                if(!MathUtil.isNear(accl.getX(), moudleAccl.getX(), Threashould) || !MathUtil.isNear(accl.getY(), moudleAccl.getY(), Threashould)){
